...
end-effectors, and sensing, but must keep one task interpretation consistent
while different robots execute concurrently in a changing workspace.
Figure~\ref{fig:scenario} grounds this problem in a conveyor setting with
complementary manipulators and three task-relevant semantic events. Language
and vision-language models can interpret instructions and ground
objects and skills from feedback~\cite{huang2023inner,liang2023code,
rana2023sayplan}. However, direct vision-language-to-action mappings do not
expose team capability constraints or responsibility interfaces, and
independent agents can maintain incompatible task views. The challenge is to
...
\begin{figure}[t]
    ...
    local execution. Top: framework and simulated conveyor workspace; middle:
    event Gantt charts; bottom: hardware workspace.}
    \label{fig:scenario}
\end{figure}
...
preserving unaffected valid execution. Contribution I is validated through
role ablation, semantic-state and role-interaction analysis, and hardware
execution; Contribution II through event experiments, affected-scope analysis,
and selective-update evaluation.
