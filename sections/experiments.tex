...
progress.
Metrics are Task Success Rate (TSR), Subtask Completion Rate (SCR),
successful-trial Makespan, Semantic Reasoning Time Ratio, and Token Usage. TSR
measures full-task success; SCR measures required primitive completion. The
semantic-time ratio is the union of reasoning intervals divided by makespan;
Token Usage is provider-reported consumption. Event Response Time spans event
detection to the next committed update. Hardware conditions use 10 trials.
...
The largest gap occurred when semantic events had to be incorporated. As
reported in Table~\ref{tab:baseline_tsr}, Ours retained 30/30 on all task-update
and disturbance cases and 27/30 under execution failure, while every baseline
lost substantially more event trials. In static \texttt{C1}, all methods except
...
\subsubsection{Failure distributions}

The failure counts in Figure~\ref{fig:failure_distribution} quantify the
...
exhibited the corresponding semantic failures. Figure~\ref{fig:failure_analysis}
shows representative cases: role ownership filters invalid dispatches and
verification exposes residual physical anomalies for event-scoped repair.

\begin{figure}[t!]
  \centering
  \input{figures/figure6_failure_analysis.tex}
...
\begin{figure}[!t]
  \centering
  \input{figures/figure7_ablation_failure_count_single.tex}
  \caption{Ablation failure counts under \texttt{C4} expose the categories
  affected by role separation and selective replanning.}
...
\input{figures/figure10_hardware_gantt.tex}

\begin{figure*}[!t]
  \centering
  \input{figures/figure8_hardware_experiments.tex}
...
\input{tables/table3_hardware_results.tex}
