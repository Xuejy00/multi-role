...
dependencies require reconsideration. The operator
$\rho_V^{\texttt{aff}}$ receives the detected event $e_k$, the latest
event-consistent instruction $\gamma$, assignment-aware graph $G$, refreshed
scene and feasibility facts $\mathcal{F}$, and verified status $z$. Verification
first applies the criterion associated with the event type. For a task update
$e^{\texttt{t}}$, it selects nodes whose stored task requirements are no longer
consistent with the revised $\gamma$. For an external disturbance
$e^{\texttt{x}}$, it compares each node's stored target, feasibility, and
dependency assumptions with the refreshed $\mathcal{F}$ and selects nodes whose
assumptions no longer hold. For an execution failure $e^{\texttt{f}}$, it
selects the primitive nodes marked $\texttt{FAILED}$ in $z$. Starting from these
directly affected nodes, verification follows their outgoing dependencies in
$G$ and adds every unfinished descendant whose validity or executability depends
on a selected node. A node is excluded from $\mathcal{N}_k^{\texttt{aff}}$ when
it satisfies neither the event-specific criterion nor this unfinished
dependency condition. Thus, independent valid nodes remain outside the affected
scope returned by~\eqref{eq:affected_subtasks}.
